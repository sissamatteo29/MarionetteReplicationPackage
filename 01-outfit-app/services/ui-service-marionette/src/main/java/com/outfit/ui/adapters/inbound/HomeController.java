...
public class HomeController {
    ...
    public HomeController(PrepareImageGalleryPageUseCase prepareImageGalleryPageUseCase, FetchProcessedImageUseCase fetchProcessedImageUseCase, UploadImageUseCase uploadImageUseCase) {
    ...
    @GetMapping("/")
    public String home(@RequestParam(defaultValue = "0") int page, @RequestParam(required = false) String success, @RequestParam(required = false) String error, Model model) {
        switch(com.outfit.ui.__marionette.BehaviourRegistry.getBehaviourId("com/outfit/ui/adapters/inbound/HomeController.java", "home")) {
            case "simple_ui":
                return home_simple_ui(page, success, error, model);
            default:
            case "beautiful_ui":
                return home_beautiful_ui(page, success, error, model);
        }
    }
    ...
    public ResponseEntity<byte[]> getEnhancedImage(@PathVariable String imageId) {
        ...
        try {
            ...
            return ResponseEntity.ok().contentType(MediaType.IMAGE_JPEG).header(HttpHeaders.CONTENT_DISPOSITION, "inline; filename=\"enhanced_" + imageId + "\"").header(HttpHeaders.CACHE_CONTROL, // Disable caching for debugging
            "no-cache, no-store, must-revalidate").header(HttpHeaders.PRAGMA, "no-cache").header(HttpHeaders.EXPIRES, "0").body(enhancedImageData);
        } catch (Exception e) {
    ...
    public String uploadImage(@RequestParam("file") MultipartFile file, Model model) {
        try {
            ...
            uploadImageUseCase.execute(file.getBytes(), filename);
    ...
    public String home_simple_ui(@RequestParam(defaultValue = "0") int page, @RequestParam(required = false) String success, @RequestParam(required = false) String error, Model model) {
    ...
    public String home_beautiful_ui(@RequestParam(defaultValue = "0") int page, @RequestParam(required = false) String success, @RequestParam(required = false) String error, Model model) {
        System.out.println("User requesting gallery page: " + page);
        try {
            // Use case: Prepare gallery page for user
            GalleryPageResponse galleryPage = prepareImageGalleryPageUseCase.fetchImageNamesForPage(page);
